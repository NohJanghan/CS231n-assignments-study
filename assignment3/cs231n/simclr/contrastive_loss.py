...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        def l(k1, k2):
            a = out[k1]
            b = out[k2]
            p = torch.exp(sim(a, b) / tau)
            denominator = 0
            for j in range(2*N):
                if j != k1:
                    denominator += torch.exp(sim(a, out[j]) / tau)
            return -torch.log(p / (denominator + 1e-8))
...
def compute_sim_matrix(out):
    ...
    # TODO: Start of your code.                                                  #
    ##############################################################################
    out_norm = torch.linalg.norm(out, dim=1)
    sim_matrix = (out @ out.T) / torch.outer(out_norm, out_norm)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return sim_matrix
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_pairs = sim_matrix[torch.arange(2*N), torch.arange(N, 3*N) % (2*N)].view(2*N, 1)
    ...
    loss = torch.sum(-torch.log(numerator / (denom + 1e-8))) / (2 * N)
